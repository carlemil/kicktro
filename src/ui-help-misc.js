  ...
  const HELP = {
    ...
    sliders: [
      ...
      { n: "Corners", w: "poly", t: "Polypinski only — how many corners the chaos game jumps between, 3 to 24, spread evenly over one sphere. 3 is the Sierpiński triangle, 4 the tetrahedron, 6 the octahedron, and higher counts keep the pattern." },
      { n: "Seed", w: "poly", t: "Polypinski only — which roll of the dice the even spread is relaxed from — it sets the orientation and, where several arrangements tie, which one. The same seed always gives the same formation, so when one looks good, note the number." },
      { n: "Random seed", w: "poly", t: "Polypinski only — Off keeps the Seed you set. On rolls a new Seed every time the effect starts (reload, scene change, switching to it) and the moment you turn it on. The number lands on the Seed slider, so when a roll looks good, turn Random seed off and that formation stays." },
      { n: "Gap", w: "poly", t: "Polypinski only — the space between the shrunken copies. At 0 each step is worked out from the corners so the copies just touch: 0.5 for the triangle, tetrahedron and octahedron, less for more corners. Positive values shrink the copies and open the holes wider; negative values let them overlap until the solid fills in." },
      ...
      { n: "Depth fade", w: "poly", t: "Polypinski only — brightness as a function of distance from you. 0 lights every point the same; positive values dim the far side of the ball (1 makes the back vanish), negative values dim the near side so the back glows through." },
