  ...
  const EFFECTS = [
    ...
    { id: "csg", name: "Smooth CSG", subtitle: "CSG · smooth-blended solids",
      help: "Three studies in constructive solid geometry, raymarched as one scene: a sphere bobbing in and out of a box and MELTING into it where they meet (smooth union), a torus spinning on its own, and a box with a spherical cavity wandering through it (smooth subtraction). Blend is the melt radius — arm its chips and every beat fuses the sphere and box into one blob before they pull apart again. Shine and Highlight width set the plastic gloss: tight and bright, or broad and soft. The camera sways about the front so the cavity stays in view.",
      params: ["csgblend", "csgspeed", "csgorbit", "csgshine", "csgwidth", "zoom", "camrx", "camry", "camrz", "fov", "palcycle", "palhold", "band", "bandsize", "banddim"], helpTags: ["all", "band"], bakesOwnZoom: true,
      draw: dt => { const s = csgSeed(dt); if (useGL) glShaderDraw("csg", u => { gl.uniform1f(u.uTime, s.t); gl.uniform1f(u.uOrbit, s.orbit); gl.uniform1f(u.uBlend, s.blend); gl.uniform1f(u.uShine, s.shine); gl.uniform1f(u.uWidth, s.width); gl.uniform1f(u.uZoom, s.zoom); }); else csgCPU(s); },
      defaults: { palcycle: [0, 0], palhold: [0, 0], zoom: [1, 1], band: [0, 0], bandsize: [1, 1], banddim: [0, 0], csgblend: [0.35, 0.35], csgspeed: [1, 1], csgorbit: [1, 1], csgshine: [1.25, 1.25], csgwidth: [0.21, 0.21] },
      beat: {}, extras: { palette: "5", morph: false } },
