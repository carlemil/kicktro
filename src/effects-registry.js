  ...
  const EFFECTS = [
    ...
    { id: "polypinski", name: "Polypinski", subtitle: "Polypinski · chaos game on a sphere",
      help: "Sierpiński set free: the chaos game played between any number of corners spread evenly over the surface of a sphere — 3 is the triangle, 4 the tetrahedron, 6 the octahedron, and more keep the pattern. Seed picks the orientation and, where several arrangements tie, which one, so a formation you like is one number you can come back to. Turn Random seed on and every start rolls a new one onto the Seed slider; turn it off when a roll looks good and it stays. The step toward each corner is worked out from the corners themselves so the shrunken copies just touch, which is what keeps the holes open at any count; Gap pushes the copies further apart (positive) or lets them overlap and fill in (negative). Wiggle lets every corner wander about its home, Wiggle speed how fast; Depth fade dims points by their distance from you; the whole solid tumbles slowly and Rotation spins it. Zoom in on the structure — the points are re-stamped at full resolution at any zoom.",
      params: ["pycorners", "pyseed", "pyrand", "pygap", "pywiggle", "pywspeed", "pydepth", "points", "size", "rot", "zoom", "camrx", "camry", "camrz", "fov", "palcycle", "palhold"],
      helpTags: ["all", "poly"], bakesOwnZoom: true, onEnter: () => pyRollSeed(),
