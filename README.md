...
- …plus more demoscene effects, each with its own sliders (react to music via the
  ...
  - **Smooth CSG** — three studies in constructive solid geometry raymarched as one scene: a sphere melting into a box where they meet (smooth union), a spinning torus, and a box with a spherical cavity wandering through it (smooth subtraction). **Blend** is the melt radius — arm its chips and each beat fuses the sphere and box into one blob; **Shine** and **Highlight width** set the plastic gloss.
